...
export const Explore = () => {
    ...
    return (
        ...
        <section className="min-h-screen flex flex-col gap-10 pt-45 px-30">
            <div className="flex flex-col items-start gap-6">
                <h1 className="text-secondary-500 text-5xl font-averia-r">Trouve ta <span className="text-main-600">plante</span></h1>
                
                <div className="flex justify-start items-center gap-1 p-2 outline-secondary-200 outline-3 rounded-md w-200 h-12 relative">
                    <svg className="w-6 h-6 fill-secondary-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M480 272C480 317.9 465.1 360.3 440 394.7L566.6 521.4C579.1 533.9 579.1 554.2 566.6 566.7C554.1 579.2 533.8 579.2 521.3 566.7L394.7 440C360.3 465.1 317.9 480 272 480C157.1 480 64 386.9 64 272C64 157.1 157.1 64 272 64C386.9 64 480 157.1 480 272zM272 416C351.5 416 416 351.5 416 272C416 192.5 351.5 128 272 128C192.5 128 128 192.5 128 272C128 351.5 192.5 416 272 416z"/></svg>
                    <input className="w-200 outline-none" type="text"></input>
                    <img src="/images/Explorer-1.png" alt="Illustration de chenille" className="w-25 absolute -top-13 right-15"></img>
                </div>

                <div className="flex flex-row justify-between gap-5">
                    <button className="btn-1 flex justify-between items-center gap-2" type="button" onClick={() => setIsFiltersOpen(true)}>
                    <svg className="w-6 h-6 fill-main-00" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M96 128C78.3 128 64 142.3 64 160C64 177.7 78.3 192 96 192L182.7 192C195 220.3 223.2 240 256 240C288.8 240 317 220.3 329.3 192L544 192C561.7 192 576 177.7 576 160C576 142.3 561.7 128 544 128L329.3 128C317 99.7 288.8 80 256 80C223.2 80 195 99.7 182.7 128L96 128zM96 288C78.3 288 64 302.3 64 320C64 337.7 78.3 352 96 352L342.7 352C355 380.3 383.2 400 416 400C448.8 400 477 380.3 489.3 352L544 352C561.7 352 576 337.7 576 320C576 302.3 561.7 288 544 288L489.3 288C477 259.7 448.8 240 416 240C383.2 240 355 259.7 342.7 288L96 288zM96 448C78.3 448 64 462.3 64 480C64 497.7 78.3 512 96 512L150.7 512C163 540.3 191.2 560 224 560C256.8 560 285 540.3 297.3 512L544 512C561.7 512 576 497.7 576 480C576 462.3 561.7 448 544 448L297.3 448C285 419.7 256.8 400 224 400C191.2 400 163 419.7 150.7 448L96 448z"/></svg>
                    Filtres
                    </button>
                ...
                {Object.keys(filters).length > 0 &&
                    ...
                    <button className="btn-3 flex justify-between items-center gap-2"  type="button" onClick={() => handleEmptyFilters()}>
                        <svg className="w-6 h-6  fill-secondary-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M320 128C263.2 128 212.1 152.7 176.9 192L224 192C241.7 192 256 206.3 256 224C256 241.7 241.7 256 224 256L96 256C78.3 256 64 241.7 64 224L64 96C64 78.3 78.3 64 96 64C113.7 64 128 78.3 128 96L128 150.7C174.9 97.6 243.5 64 320 64C461.4 64 576 178.6 576 320C576 461.4 461.4 576 320 576C233 576 156.1 532.6 109.9 466.3C99.8 451.8 103.3 431.9 117.8 421.7C132.3 411.5 152.2 415.1 162.4 429.6C197.2 479.4 254.8 511.9 320 511.9C426 511.9 512 425.9 512 319.9C512 213.9 426 128 320 128z"/></svg>
            ...
            {isLoading ? (
                ...
                (plants !== null) ? (
                    <ul className="flex flex-wrap justify-center items-start gap-6">
                        {plants.map(plant => (<PlantCard key={plant._id} plant={plant}/>))}
                    </ul>
                    
                ) : (
                    <p>Il semblerait qu'il y ait un bug, impossible de charger les plantes pour l'instant.</p>
                )
            }
